...
function readContacts() { try { return JSON.parse(localStorage.getItem(CONTACTS_KEY) || "[]"); } catch { return []; } }
function writeContacts(d) { localStorage.setItem(CONTACTS_KEY, JSON.stringify(d)); try { window.dispatchEvent(new Event("fb:data-changed")); } catch { } }

const SUPPORTS_KEY = "fb_supports";
function readSupports() { try { return JSON.parse(localStorage.getItem(SUPPORTS_KEY) || "[]"); } catch { return []; } }
function writeSupports(d) { localStorage.setItem(SUPPORTS_KEY, JSON.stringify(d)); try { window.dispatchEvent(new Event("fb:data-changed")); } catch { } }
