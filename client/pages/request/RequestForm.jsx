            ...
            {!success ? (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border/60 bg-background p-6 shadow-sm">
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <Label>Urgency</Label>
                            <Select value={form.details.urgency} onValueChange={(v) => setForm((f) => ({ ...f, details: { ...f.details, urgency: v } }))}>
                                <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                                <SelectContent>
                        ...
                        <div className="sm:col-span-2">
                            ...
                            <ErrorBoundary>
                                <AddressInput
                                    ...
                                    onLocationSelect={(location) => {
                                        console.log("📍 RequestForm received location:", location);
                                        setForm((f) => ({
                                            ...f,
                                            location: {
                                                ...f.location,
                            ...
                            <MapPicker className="w-full h-[300px]" value={{ lat: form.location.lat, lng: form.location.lng }} onChange={(pos) => setForm((f) => ({ ...f, location: { ...f.location, ...pos } }))} />
                        </div>
                    </div>
                    <div className="mt-6">
